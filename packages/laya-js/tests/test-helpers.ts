...
import type { ChoiceQuestion, NoulQuestion, Questions, ScoreQuestion } from '../src/laya-types.ts';
import { VECTORS_DIR } from '../src/laya-paths.ts';

/** The 5 parity fixtures (sorted). Vectors dir also holds act_head/bpe-fuzz/npz — never glob it. */
export const FIXTURE_NAMES = ['choice-2', 'choice-3', 'choice-6', 'mixed-batch', 'orig-3q'];

export const fixturePath = (name: string): string => join(VECTORS_DIR, `${name}.json`);

export const vectorsFile = (name: string): string => join(VECTORS_DIR, name);
