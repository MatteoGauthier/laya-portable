...
import {
  ...
  collateItems,
  QTYPES,
} from '../src/laya-preprocess.ts';
import { softmax, confidenceFromProbs, tempBucket, predictFromLogits } from '../src/laya-postprocess.ts';
import { actionLogits, gelu, erf } from '../src/laya-action.ts';
import { toI64, toB8, toFeedData, splitOutputs } from '../src/laya-feed.ts';
import { STATE, questionsFor, vectorsFile } from './test-helpers.ts';
import { DEFAULT_TOKENIZER } from '../src/laya-paths.ts';
import type { BuiltItem, CollatedBatch, TemperatureConfig, TokenizerJson } from '../src/laya-types.ts';

const tokJson = JSON.parse(readFileSync(DEFAULT_TOKENIZER, 'utf8')) as TokenizerJson;
...
describe('bpe', () => {
  it('matches fuzz subset offline', () => {
    const tok = loadBpeTokenizer(tokJson);
    const cases = JSON.parse(readFileSync(vectorsFile('bpe-fuzz.json'), 'utf8')) as {
      text: string;
      ids: number[];
    }[];
