...
import { toInternal, buildSequence, collateItems, QTYPES } from '@js/laya-preprocess.ts';
import { predictFromLogits } from '@js/laya-postprocess.ts';
import { actionLogits } from '@js/laya-action.ts';
import { toFeedData, splitOutputs } from '@js/laya-feed.ts';
import type {
  ActHeadWeights,
  BuiltItem,
  CollatedBatch,
  ...
  void (async () => {
    ...
    try {
      ...
      if (!sess || readyModel !== modelTag) {
        ...
        actW = (await (await fetch('/js/act_head.json')).json()) as ActHeadWeights;
      }
      const session = sess;
      const weights = actW;
