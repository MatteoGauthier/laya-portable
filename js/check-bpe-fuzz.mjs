...
for (let i = 0; i < cases.length; i++) {
  ...
  let got;
  try { got = tok.encode(text); }
  catch (e) { fails.push([i, JSON.stringify(text.slice(0, 60)), 'THROW ' + e.message.slice(0, 80)]); continue; }
  if (JSON.stringify(got) === JSON.stringify(exp)) pass++;
  else fails.push([i, JSON.stringify(text.slice(0, 60)), `exp[${exp.slice(0, 8)}] len ${exp.length} vs got[${got.slice(0, 8)}] len ${got.length}`]);
}
