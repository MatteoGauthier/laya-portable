...
# Reports live next to the pipeline until step 4 moves them to packages/test-vectors.
REPORTS = ROOT / "packages" / "test-vectors" / "reports"


def load(name):
    p = REPORTS / name
    assert p.exists(), f"missing {p} — run tools/export/check_parity.py first"
...
def test_accuracy_thresholds():
    p = REPORTS / "accuracy-report.json"
    if not p.exists():
        import pytest
